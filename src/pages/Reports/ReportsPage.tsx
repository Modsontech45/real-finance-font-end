...
const ReportsPage: React.FC = () => {
  ...
  const getTransactionDate = (t) => {
    const dateValue = t.date || t.created_at || t.createdAt || t.transaction_date || t.transactionDate || t.updatedAt;
...
const filteredTransactions = transactions.filter((t) => {
 ...
 console.log('Filtering transactions from', start, 'to', end,'this is the filtered transactions:', filteredTransactions);
  // Metrics
  ...
  const highestIncome = filteredTransactions
    .filter(t => t.type === 'income')
    .reduce((max, t) => parseFloat(t.amount) > parseFloat(max.amount) ? t : max, { amount: 0, name: 'No income' });

  const highestExpense = filteredTransactions
    .filter(t => t.type === 'expense')
    .reduce((max, t) => parseFloat(t.amount) > parseFloat(max.amount) ? t : max, { amount: 0, name: 'No expenses' });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 rounded-2xl p-8 text-white">
        ...
        <p className="text-emerald-100 text-lg">Analyze your financial performance with detailed insights</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          {/* Profit Margin */}
          <Card className={`bg-gradient-to-br ${profitMargin >= 0 ? 'from-purple-500 to-purple-600' : 'from-gray-500 to-gray-600'} text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
            <div className="flex items-center justify-between">
              <div>
          ...
          <Card className="shadow-lg border-0 bg-gradient-to-br from-white to-green-50">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">${parseFloat(highestIncome.amount).toLocaleString()}</p>
          ...
          <Card className="shadow-lg border-0 bg-gradient-to-br from-white to-red-50">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">${parseFloat(highestExpense.amount).toLocaleString()}</p>
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="shadow-lg border-0 bg-black">
              <h3 className="text-lg font-semibold text-white mb-4">Monthly Income vs Expenses ({currentYear})</h3>
          ...
          <Card className="shadow-lg border-0 bg-black">
            <h3 className="text-lg font-semibold text-white mb-4">Profit Trend ({currentYear})</h3>
            <ResponsiveContainer width="100%" height={300}>
